// Casting과 N차원
// Casting
// (원하는 캐스팅 종류)<바꾸려는 타입>대상
// ex) (int)3.14 @ C
//     static_cast<int>(3.14) @ C++
//     int(3.14) @ C++

// N차원 Class
// N차원을 Array로 구현하기 위해, dim과 arraysize를 사용한다.
// dim은 차원의 수, arraysize는 각 차원의 크기를 나타낸다.
...
namespace MyArray {
    ...
    public:
        ...
        void initialize_address(Address* current) {
            if (!current) return; // current 가 NULL 일 경우 return
...
int main() {
    int size[] = { 2, 3, 4 }; // 3차원 배열
    MyArray::Array arr(3, size); // 3차원 배열 생성

    MyArray::Array::Iterator itr = arr.begin(); // begin() 은 Iterator 를 반환한다.

    
    for (int i = 0; itr != arr.end(); itr++, i++) (*itr) = i; // 0 부터 23 까지 저장
    for (itr = arr.begin(); itr != arr.end(); itr++) 
        std::cout << *itr << std::endl; // 0 부터 23 까지 출력
